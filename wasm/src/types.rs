...
// ---------------------------------------------------------------------------
// Step 1 — serializer helpers + byte/key converters
// ---------------------------------------------------------------------------

use datalayer_driver::{Bytes32, Program, PublicKey, SecretKey, Signature};
...
pub fn secret_key(buf: &[u8]) -> Result<SecretKey, JsValue> {
    let arr =
        <[u8; 32]>::try_from(buf).map_err(|_| JsValue::from_str("expected 32-byte secret key"))?;
...
pub struct DataStoreMetadata {
    ...
    /// 32-byte size proof as a `Uint8Array` (mirrors NAPI's `Option<Buffer>`).
    /// The native type stores this as an `Option<String>` hex representation,
...
impl DataStoreMetadata {
    pub fn to_native(&self) -> Result<RustDataStoreMetadata, JsValue> {
        Ok(RustDataStoreMetadata {
            ...
            bytes: self.bytes,
            // JS passes 32 raw bytes; the native type stores the hex string.
    ...
    pub fn from_native(m: &RustDataStoreMetadata) -> Result<Self, JsValue> {
        Ok(DataStoreMetadata {
            ...
            bytes: m.bytes,
            // Native stores hex; decode back to the 32 raw bytes for JS.
...
/// Represents one of three delegated-puzzle variants.
/// Exactly one of the three option groups should be populated.
#[derive(Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
// serde_bytes doesn't support Option<Vec<u8>> directly; provide a local module.
mod serde_bytes_opt {
...
impl DelegatedPuzzle {
    pub fn to_native(&self) -> Result<RustDelegatedPuzzle, JsValue> {
        if let Some(h) = &self.admin_inner_puzzle_hash {
            // Admin variant wraps TreeHash; Bytes32 converts to TreeHash via Into.
    ...
    pub fn from_native(d: &RustDelegatedPuzzle) -> Result<Self, JsValue> {
        Ok(match d {
            RustDelegatedPuzzle::Admin(th) => {
                // TreeHash → Bytes32 via Into, then .to_vec()
...
// ---------------------------------------------------------------------------
// Step 5 — ServerCoin / NewServerCoin / Output boundary structs
// ---------------------------------------------------------------------------
...
/// Output for send_xch.
/// JS field names (camelCase via serde): puzzleHash, amount, memos.
/// Matches NAPI's `Output { puzzle_hash, amount, memos }` which NAPI maps to
/// the same camelCase names.
#[derive(Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Output {
    #[serde(with = "serde_bytes")]
    pub puzzle_hash: Vec<u8>,
    pub amount: u64,
    pub memos: Vec<serde_bytes::ByteBuf>,
}
