...
pub fn new_lineage_proof(lineage_proof: crate::js::LineageProof) -> crate::js::Proof {
    crate::js::Proof {
    ...
    fn from_js(value: DelegatedPuzzle) -> Result<Self> {
        Ok(
                ...
                (value.oracle_payment_puzzle_hash, value.oracle_fee)
            {
                ...
                return Err(crate::js::err(ConversionError::MissingDelegatedPuzzleInfo));
            },
        )
    }
}
...
impl Tls {
    ...
    pub fn new(cert_path: String, key_path: String) -> napi::Result<Self> {
        let cert = load_ssl_cert(&cert_path, &key_path).map_err(crate::js::err)?;
        let tls = create_native_tls_connector(&cert).map_err(crate::js::err)?;
        Ok(Self(tls))
    }
}
...
impl Peer {
    ...
    pub async fn new(node_uri: String, peer_type: PeerType, tls: &Tls) -> napi::Result<Self> {
        ...
        if peer_type == PeerType::Simulator {
            let simulator = PeerSimulator::new().await.map_err(crate::js::err)?;
            let (p, r) = simulator.connect_raw().await.map_err(crate::js::err)?;
            peer = p;
            receiver = r;
            ...
            let (p, r) = connect_peer(
                ...
                if let Ok(socket_addr) = node_uri.parse::<SocketAddr>() {
                    ...
                    return Err(crate::js::err(ConversionError::InvalidUri(node_uri)));
                },
                PeerOptions::default(),
            )
            .await
    ...
    pub async fn wait_for_coin_to_be_spent(
        ...
        if spent_height.is_none() {
            ...
            receiver
                .recv()
                .await
                .ok_or_else(|| crate::js::err("Failed to receive spent notification"))?;
        }
    ...
    pub async fn connect_random(peer_type: PeerType, tls: &Tls) -> napi::Result<Self> {
        ...
        Err(crate::js::err("Unable to connect to any discovered peer"))
    }
}
...
pub fn select_coins(all_coins: Vec<Coin>, total_amount: BigInt) -> napi::Result<Vec<Coin>> {
    let coins: Vec<RustCoin> = all_coins
        ...
        .collect::<Result<Vec<RustCoin>>>()?;
    let selected_coins =
        wallet::select_coins(coins, u64::from_js(total_amount)?).map_err(crate::js::err)?;
...
pub fn create_server_coin(
    ...
    let result = wallet::create_server_coin(
        RustPublicKey::from_js(synthetic_key)?,
        selected_coins
            .into_iter()
            .map(RustCoin::from_js)
    ...
    Ok(NewServerCoin {
        coin_spends: result
            ...
            .map(|c| c.to_js())
            .collect::<Result<Vec<CoinSpend>>>()?,
        server_coin: result.server_coin.to_js()?,
    })
}
...
pub fn mint_store(
    ...
    let response = wallet::mint_store(
        ...
        delegated_puzzles
            ...
            .map(RustDelegatedPuzzle::from_js)
            .collect::<Result<Vec<RustDelegatedPuzzle>>>()?,
        u64::from_js(fee).map_err(crate::js::err)?,
    )
    .map_err(crate::js::err)?;
...
pub fn hex_spend_bundle_to_coin_spends(hex: String) -> napi::Result<Vec<CoinSpend>> {
    let bytes = hex::decode(hex).map_err(crate::js::err)?;
    let spend_bundle = RustSpendBundle::from_bytes(&bytes).map_err(crate::js::err)?;
    spend_bundle
        .coin_spends
        .into_iter()
        .map(|cs| cs.to_js())
...
pub fn spend_bundle_to_hex(spend_bundle: SpendBundle) -> napi::Result<String> {
    ...
    let bytes = rust_spend_bundle.to_bytes().map_err(crate::js::err)?;
    Ok(hex::encode(bytes))
}
...
pub fn create_simple_did(
    ...
    selected_coins: Vec<Coin>,
    fee: BigInt,
) -> napi::Result<crate::js::CreateDidResult> {
    let synthetic_key = RustPublicKey::from_js(synthetic_key)?;
    let selected_coins = selected_coins
        .into_iter()
    ...
    let (coin_spends, did_coin) =
        wallet::create_simple_did(synthetic_key, selected_coins, fee).map_err(crate::js::err)?;
...
pub async fn simulator_new_puzzle(value: BigInt) -> napi::Result<crate::js::SimulatorPuzzle> {
    let value = u64::from_js(value)?;
    let (puzzle_hash, puzzle_reveal) = to_puzzle(value).map_err(crate::js::err)?;
    Ok(crate::js::SimulatorPuzzle {
...
pub fn simulator_new_blspair(value: BigInt) -> napi::Result<crate::js::BlsPair> {
    let value = u64::from_js(value)?;
